...
from buzz import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, MESSAGE_DUMP, LOGGER
from buzz.modules.helper_funcs.handlers import CMD_STARTERS
from buzz.modules.helper_funcs.misc import is_module_loaded, send_to_list
from buzz.modules.helper_funcs.chat_status import is_user_admin
from buzz.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from buzz.modules.helper_funcs.string_handling import markdown_parser
from buzz.modules.disable import DisableAbleCommandHandler

import buzz.modules.sql.feds_sql as sql

# Hellol

LOGGER.info("Federation reworked by Ridwan (@psycho_syridwan) on Telegram.")
...
def new_fed(bot: Bot, update: Update):
	...
	if not fednam == '':
		...
		if not x:
			update.effective_message.reply_text("Federation creation failed! Keep in the mind that this rarely happened! Ask in @ossuport for help!")
			return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
	...
	if fban:
		...
		if not x:
			message.reply_text("Failed to ban from the federation! If this problem continues, contact @psycho_syridwan.")
			return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
	...
	if len(args) >= 1:
		...
		if not x:
			update.effective_message.reply_text("Big F! There is an error while setting federation rules! If you wondered why please ask it in @psycho_syridwan !")
			return
