...
import buzz.modules.sql.notes_sql as sql
from buzz import dispatcher, LOGGER, OWNER_ID, SUDO_USERS, MESSAGE_DUMP
from buzz.__main__ import DATA_IMPORT
from buzz.modules.helper_funcs.chat_status import user_admin
from buzz.modules.helper_funcs.misc import build_keyboard, revert_buttons
from buzz.modules.helper_funcs.msg_types import get_note_type
from buzz.modules.rules import get_rules
import buzz.modules.sql.rules_sql as rulessql
from buzz.modules.sql import warns_sql as warnssql
import buzz.modules.sql.blacklist_sql as blacklistsql
from buzz.modules.sql import disable_sql as disabledsql
from buzz.modules.sql import cust_filters_sql as filtersql
import buzz.modules.sql.welcome_sql as welcsql
import buzz.modules.sql.locks_sql as locksql
...
def import_data(bot: Bot, update):
	...
	if msg.reply_to_message and msg.reply_to_message.document:
		...
		try:
			...
			return msg.reply_text("There is problem while importing the data! Please ask in @ossuport about why this happened.")
		# Check if backup is from self
		try:
			if str(bot.id) != str(data[str(chat.id)]['bot']):
				return msg.reply_text("Backup from another bot that is not suggested might cause the problem, documents, photos, videos, audios, records might not work as it should be. However, You can still request a feature regarding this in @ossuport !")
		except:
			pass
		# Select data source
		...
		try:
			...
			msg.reply_text("An error occurred while recovering your data. The process failed. If you experience a problem with this, please ask in @ossuport . My owner and community will be happy to help. Also, bugs report makes me even better!\nThank you!")
...
def export_data(bot: Bot, update: Update, chat_data):
	...
	f=open("buzzb{}.backup".format(chat_id), "w")
	...
	try:
		...
		pass
	bot.sendDocument(current_chat_id, document=open('buzzb{}.backup'.format(chat_id), 'rb'), caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `buzzb` is specially made for notes.".format(chat.title, chat_id, tgl), timeout=360, reply_to_message_id=msg.message_id, parse_mode=ParseMode.MARKDOWN)
	os.remove("buzzb{}.backup".format(chat_id)) # Cleaning file
...
This module is still in beta! Report bugs in @ossuport !
"""
